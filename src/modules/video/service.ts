...
export const videoService = {
  ...
  async presignUpload(videoId: string, contentType: string) {
    ...
    const dbU = getDb();
    await dbU.update(videos).set({ rawKey: key }).where(eq(videos.id, videoId));
  ...
  async confirmUpload(videoId: string) {
    ...
    const db1 = getDb();
    await db1.update(videos).set({ status: "processing", error: null, updatedAt: new Date() }).where(eq(videos.id, videoId));
  ...
  async assertCanStream(userId: string, videoId: string, userRoles: string[]) {
    ...
    const db2 = getDb();
    const lessonRows = await db2.select({ id: lessons.id, courseId: lessons.courseId, isFree: lessons.isFree }).from(lessons).where(and(eq(lessons.videoId, videoId), notDeleted(lessons.deletedAt)));
    ...
    for (const lesson of lessonRows) {
      ...
      const course = await db2.select({ id: courses.id, mentorId: courses.mentorId }).from(courses).where(eq(courses.id, lesson.courseId)).limit(1);
      const c = course[0];
      if (!c) continue;
      if (userRoles.includes("admin") || c.mentorId === userId) return;
      const enrollment = await db2.select({ id: courseEnrollments.id }).from(courseEnrollments).where(and(eq(courseEnrollments.userId, userId), eq(courseEnrollments.courseId, c.id))).limit(1);
